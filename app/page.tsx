    ...
    <div className="min-h-screen bg-[#1a1a1a] flex flex-col">
          ...
          <div className="mt-6 flex justify-center items-center gap-4 sm:gap-8">
            {Array.from({ length: totalSteps }).map((_, i) => (
              <div key={i} className="flex flex-col items-center">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold border-2 transition-all ${i < step
                ...
                {i < totalSteps - 1 && (
                  <div
                    className={`h-1 w-12 sm:w-20 mt-2 ${i < step - 1 ? 'bg-[#00ff88]' : 'bg-gray-700'
                      }`}
                  />
                )}
              </div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-50 bg-[#1a1a1a] border-t border-[#00ff88]/30 py-6 px-4 sm:px-6">
        <button
          ...
          className={`w-full font-semibold py-3 sm:py-4 px-6 rounded-full text-base sm:text-lg transition-all cursor-pointer ${isNextDisabled
